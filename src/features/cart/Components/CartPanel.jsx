...
function CartPanel({ cart, onChangeQty, onRemove, total, cashSessionOpen = true }) {
    ...
    return (
        <div className="w-32 sm:w-44 md:w-64 lg:w-80 shrink-0 h-full flex flex-col bg-white dark:bg-slate-900 border border-gray-200 dark:border-transparent rounded-xl p-1.5 sm:p-3 lg:p-4 text-gray-900 dark:text-white overflow-hidden">
            ...
            <div className="flex-1 min-h-[80px] sm:min-h-[120px] overflow-y-auto soft-scrollbar mt-2 sm:mt-4">
                {cart.length === 0 ? (
                    <div className="flex flex-col items-center justify-center text-center py-4 sm:py-10 text-gray-400 dark:text-slate-500">
                    ...
                    <ul className="space-y-2 sm:space-y-3">
                        {cart.map((item) => (
                            <li key={item.id} className="flex items-center justify-between gap-1 sm:gap-2">
                                <button
                                    type="button"
                                    onClick={() => setEditingItem(item)}
                                    className="min-w-0 text-left hover:opacity-80"
                                >
                                    <div className="text-[11px] sm:text-sm font-medium truncate">{item.name}</div>
                                ...
                                <div className="flex items-center gap-0.5 sm:gap-1 shrink-0">
                                    <button onClick={() => onChangeQty(item.id, -1)} className="p-0.5 sm:p-1 rounded hover:bg-gray-100 dark:hover:bg-slate-700">
                                        <Minus size={12} className="sm:w-3.5 sm:h-3.5" />
                                    </button>
                                    <button onClick={() => onChangeQty(item.id, 1)} className="p-0.5 sm:p-1 rounded hover:bg-gray-100 dark:hover:bg-slate-700">
                                        <Plus size={12} className="sm:w-3.5 sm:h-3.5" />
                                    </button>
                                    <button onClick={() => onRemove(item.id)} className="p-0.5 sm:p-1 rounded hover:bg-gray-100 dark:hover:bg-slate-700 text-red-500 dark:text-red-400">
